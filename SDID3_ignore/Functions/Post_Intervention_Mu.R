...
Post_Intervention_Mu <- function(
  ...
  synthetic_linear_pred <- intercept_opt + as.vector(controls_post %*% optimal_weights)
  synthetic_counts.post <- exp(synthetic_linear_pred)
  synthetic_linear_pred <- intercept_opt + as.vector(controls_pre %*% optimal_weights)
  synthetic_counts.pre <- exp(synthetic_linear_pred)
  
  # Inputs
  y_pre <- synthetic_counts.pre
  y_post_mean <- mean(synthetic_counts.post)
